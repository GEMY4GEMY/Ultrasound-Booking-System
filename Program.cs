...
app.MapPost("/api/bookings/{id}/cancel",(string id,string user)=>{var a=Read<Booking>(B);var x=a.FirstOrDefault(z=>z.id==id);if(x==null)return Results.NotFound();x.status="Cancelled";x.modifiedBy=user;x.updatedAt=DateTime.Now;Write(B,a);Log(user,"CANCEL BOOKING",id+" - "+x.patientName);return Results.Ok(x);});
...
app.MapGet("/api/health",()=>Results.Ok(new{status="ok",server=Environment.MachineName,time=DateTime.Now,bookings=Read<Booking>(B).Count,users=Read<User>(U).Count,dataPath=dir}));
app.MapGet("/api/backup",()=>{var stamp=CreateBackup();Log("SYSTEM","BACKUP",stamp);return Results.Ok(new{folder=stamp});});
app.MapGet("/api/backups",()=>{var root=Path.Combine(dir,"backups");Directory.CreateDirectory(root);return Results.Json(Directory.GetDirectories(root).Select(Path.GetFileName).OrderByDescending(x=>x));});
app.MapPost("/api/restore/{stamp}",(string stamp,string user)=>{var root=Path.Combine(dir,"backups");var bd=Path.Combine(root,Path.GetFileName(stamp));if(!Directory.Exists(bd))return Results.NotFound();CreateBackup();foreach(var target in new[]{B,U,D,E,A}){var src=Path.Combine(bd,Path.GetFileName(target));if(File.Exists(src))File.Copy(src,target,true);}Log(user,"RESTORE BACKUP",stamp);return Results.Ok();});
var autoStamp=DateTime.Now.ToString("yyyyMMdd");var marker=Path.Combine(dir,"backup_"+autoStamp+".done");if(!File.Exists(marker)){CreateBackup();File.WriteAllText(marker,DateTime.Now.ToString("O"));}
app.Run();
...
string CreateBackup(){var stamp=DateTime.Now.ToString("yyyyMMdd_HHmmss");var bd=Path.Combine(dir,"backups",stamp);Directory.CreateDirectory(bd);foreach(var f in new[]{B,U,D,E,A})if(File.Exists(f))File.Copy(f,Path.Combine(bd,Path.GetFileName(f)),true);return stamp;}
...
class Booking{public string id{get;set;}="";public string patientName{get;set;}="";public string patientId{get;set;}="";public string phone{get;set;}="";public string exam{get;set;}="";public string doctor{get;set;}="";public DateTime bookingDate{get;set;}public string shift{get;set;}="Morning";public string status{get;set;}="New";public bool papersReceived{get;set;}public string notes{get;set;}="";public string bookedBy{get;set;}="";public string modifiedBy{get;set;}="";public DateTime createdAt{get;set;}public DateTime updatedAt{get;set;}}